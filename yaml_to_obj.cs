// Dafny program yaml_to_obj.dfy compiled into C#
...
// Command-line arguments: build --target cs yaml_to_obj.dfy ExternalFunctions.cs
// yaml_to_obj.dfy
...
module YamlToDafnyTranslator {
  ...
  method {:extern ""YamlToDafnyTranslator"", ""ParseYaml""} ParseYaml(lines: seq<string>) returns (tables: seq<Table>)
    decreases lines

  method {:extern ""YamlToDafnyTranslator"", ""ReadAllLines""} ReadAllLines(path: string) returns (lines: seq<string>)
    decreases path

  method {:extern ""YamlToDafnyTranslator"", ""WriteLine""} WriteLine(s: string)
    decreases s
  ...
  public partial class __default {
    ...
    public static void _Main(Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> args)
    {
      if ((new BigInteger((args).Count)).Sign == 0) {
        YamlToDafnyTranslator.WriteLine(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("Usage: yaml_to_dafny <file1.yml> [<file2.yml> ...]"));
        return ;
      }
      BigInteger _0_idx;
      _0_idx = BigInteger.Zero;
      while ((_0_idx) < (new BigInteger((args).Count))) {
        Dafny.ISequence<Dafny.Rune> _1_path;
        _1_path = (args).Select(_0_idx);
        Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> _2_content;
        Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> _out0;
        _out0 = YamlToDafnyTranslator.ReadAllLines(_1_path);
        _2_content = _out0;
        Dafny.ISequence<YamlToDafnyTranslator._ITable> _3_tables;
        Dafny.ISequence<YamlToDafnyTranslator._ITable> _out1;
        _out1 = YamlToDafnyTranslator.ParseYaml(_2_content);
        _3_tables = _out1;
        BigInteger _4_j;
        _4_j = BigInteger.Zero;
        while ((_4_j) < (new BigInteger((_3_tables).Count))) {
          YamlToDafnyTranslator.WriteLine(YamlToDafnyTranslator.__default.ClassAsString((_3_tables).Select(_4_j)));
          _4_j = (_4_j) + (BigInteger.One);
        }
        _0_idx = (_0_idx) + (BigInteger.One);
      }
    }
  }
...
class __CallToMain {
  public static void Main(string[] args) {
    Dafny.Helpers.WithHaltHandling(() => YamlToDafnyTranslator.__default._Main(Dafny.Sequence<Dafny.ISequence<Dafny.Rune>>.UnicodeFromMainArguments(args)));
  }
}
